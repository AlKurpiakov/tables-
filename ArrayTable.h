...
public:
    ...
    ArrayTable(size_t tabSize = 100) {
        _tabSize = tabSize;
        _records = new PTabRecord[tabSize];
    ...
    size_t GetSizeTable() { return _tabSize; }
    virtual bool IsFull() const { return _dataCount >= _tabSize; }

    virtual Key GetKey() const {return GetKey(DataPos::Current); }
    virtual PDataValue GetValue() const { return GetValue(DataPos::Current); }
